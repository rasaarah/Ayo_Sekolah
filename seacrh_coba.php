<?php
  ...
  function build_query($user_search) {
    $search_query1 = "SELECT * FROM sekolah INNER JOIN kecamatan ON sekolah.id_kecamatan=kecamatan.id_kecamatan ";
    ...
    if (!empty($where_clause)) { 
      $search_query1 .= " WHERE $where_clause";
    }
  ...
  function generate_page_links($user_search, $cur_page, $num_pages) {
    ...
    for ($i = 1; $i <= $num_pages; $i++) {
      if ($cur_page == $i) {
        ...
        $page_links .= '<li><a href="' . $_SERVER['PHP_SELF'] . '?cari=' . $user_search . '&page=' . $i . '"> ' . $i . '</a></li>';
      }
    }

    // Jika halaman ini tidak halaman terakhir, menghasilkan "berikutnya" link
    if ($cur_page < $num_pages) {
      $page_links .= '<li><a href="' . $_SERVER['PHP_SELF'] . '?cari=' . $user_search . '&page=' . ($cur_page + 1) . '">-></a></li>';
    }
    else {
