<?php
  ...
  function build_query($user_search) {
    $search_query = "SELECT * FROM sd INNER JOIN kecamatan ON sd.id_kecamatan=kecamatan.id_kecamatan ";
  ...
  while ($row = mysqli_fetch_array($result)) {
      ...
      if($row['status']==0)
        ...
        echo '<p><a href="maps_sd.php?page=maps&id_tk='.$row['id_sd'].'" >Lihat maps</a></p>';
      echo'</div>';
      echo'  </div>';
   echo'   <hr>';
  } 
